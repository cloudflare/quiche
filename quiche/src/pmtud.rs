...
#[derive(Clone, Copy)]
enum SuccessfulProbe {
    CurrentSearch(usize),
    PreviousSearch(usize),
}

impl SuccessfulProbe {
    fn size(self) -> usize {
        match self {
            Self::CurrentSearch(size) | Self::PreviousSearch(size) => size,
        }
    }
}
...
pub struct Pmtud {
    ...
    /// The largest successful probe and whether it was confirmed in the current
    /// Search Phase.
    largest_successful_probe: Option<SuccessfulProbe>,
...
impl Pmtud {
    ...
    pub fn get_current_mtu(&self) -> usize {
        self.largest_successful_probe
            .map_or(MIN_PLPMTU, SuccessfulProbe::size)
    }
    ...
    fn update_probe_size(&mut self, now: Instant) {
        match (
            ...
            (
                Some(failed_probe_size),
                Some(SuccessfulProbe::CurrentSearch(successful_probe_size)),
            ) => {
            ...
            (
                Some(failed_probe_size),
                Some(SuccessfulProbe::PreviousSearch(successful_probe_size)),
            ) =>
                if failed_probe_size <= successful_probe_size {
                    self.largest_successful_probe = None;
                    self.probe_size = (MIN_PLPMTU + failed_probe_size) / 2;
                } else if failed_probe_size - successful_probe_size <= 1 {
            ...
            (None, Some(SuccessfulProbe::PreviousSearch(_))) =>
    ...
    pub fn successful_probe(
        ...
        let successful_probe_size = self
            .largest_successful_probe
            .map_or(probe_size, |probe| probe.size().max(probe_size))
            .min(self.maximum_supported_mtu);
        self.largest_successful_probe =
            Some(SuccessfulProbe::CurrentSearch(successful_probe_size));
        ...
        self.largest_successful_probe.map(SuccessfulProbe::size)
    }
    ...
    fn enter_search_phase(&mut self) {
        ...
        self.largest_successful_probe = self
            .largest_successful_probe
            .map(|probe| SuccessfulProbe::PreviousSearch(probe.size()));
        self.pmtu = None;
        self.probe_failure_count = 0;
...
mod tests {
    ...
    fn pmtud_revalidation_tolerates_random_packet_loss() {
        ...
        assert!(pmtud.largest_successful_probe.is_none());
    ...
    fn pmtud_revalidation_failure_binary_searches_not_restarts() {
        ...
        // Revalidation clears largest_successful_probe
        pmtud.revalidate_pmtu();
        assert!(pmtud.largest_successful_probe.is_none());
    ...
    fn pmtud_raise_search_keeps_serving_confirmed_mtu() {
        ...
        while pmtud.get_probe_size() > 1400 {
            assert_eq!(pmtud.get_current_mtu(), 1400);
            let probe_size = pmtud.get_probe_size();
            fail_probe_max_times(&mut pmtud, probe_size);
        }
    ...
    #[test]
    fn pmtud_raise_search_discovers_lower_pmtu() {
        let interval = Duration::from_secs(600);
        let mut pmtud = Pmtud::new(1353, 1, Some(interval));

        pmtud_test_runner(&mut pmtud, 1312);

        let deadline = pmtud.raise_timer().expect("raise timer armed");
        pmtud.on_raise_timeout(deadline);

        while pmtud.get_probe_size() > 1312 {
            assert_eq!(pmtud.get_current_mtu(), 1312);
            let probe_size = pmtud.get_probe_size();
            fail_probe_max_times(&mut pmtud, probe_size);
        }

        assert!(pmtud.should_probe());
        assert_eq!(pmtud.get_probe_size(), 1312);
        pmtud.failed_probe(1312, Instant::now());
        pmtud_test_runner(&mut pmtud, 1272);
    }
