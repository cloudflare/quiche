...
use crate::recovery::congestion::bbr::BandwidthSampler;
use crate::recovery::congestion::Lost;
use crate::recovery::congestion::RttStats;
use crate::recovery::congestion::INITIAL_RTT;
    ...
    fn on_packets_acked(&mut self, last_acked_packet: u64) -> bool {
        match self.end_of_round_trip {
            ...
            _ => {
                ...
                self.end_of_round_trip = Some(last_acked_packet);
                true
            },
        }
    }
...
impl BBRv2NetworkModel {
    ...
    fn adapt_lower_bounds(&mut self, congestion_event: &BBRv2CongestionEvent) {
        ...
        match PARAMS.bw_lo_mode {
            ...
            BwLoMode::MinRttReduction => {
                ...
                if let Some(lo) = self.bandwidth_lo {
                    if reduction < lo {
                        self.bandwidth_lo = lo - reduction
                    }
                }
            },
    ...
    fn queueing_threshold_extra_bytes(&self) -> usize {
        // TODO(vlad): 2 * mss
        2 * 1300
    }
    ...
    pub(super) fn cap_inflight_lo(&mut self, cap: usize) {
        if self.inflight_lo != usize::MAX {
            self.inflight_lo = cap.max(self.inflight_lo)
        }
    }
