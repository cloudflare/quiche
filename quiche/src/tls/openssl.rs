...
const OSSL_RECORD_PROTECTION_LEVEL_NONE: c_uint = 0;
const OSSL_RECORD_PROTECTION_LEVEL_EARLY: c_uint = 1;
const OSSL_RECORD_PROTECTION_LEVEL_HANDSHAKE: c_uint = 2;
const OSSL_RECORD_PROTECTION_LEVEL_APPLICATION: c_uint = 3;

#[repr(C)]
#[allow(non_camel_case_types)]
pub(super) struct OSSL_DISPATCH {
    function_id: c_int,
    function: Option<unsafe extern "C" fn()>,
}

pub(super) static QTDIS: &[OSSL_DISPATCH] = &[
    OSSL_DISPATCH {
        function_id: OSSL_FUNC_SSL_QUIC_TLS_CRYPTO_SEND,
        function: Some(unsafe { transmute(
            crypto_send_cb as unsafe extern "C"
            fn(*mut SSL, *const u8, usize, *mut usize, *mut c_void) -> i32
        )}),
    },
    OSSL_DISPATCH {
        function_id: OSSL_FUNC_SSL_QUIC_TLS_CRYPTO_RECV_RCD,
        function: Some(unsafe { transmute(
            crypto_recv_cb as unsafe extern "C"
            fn(*mut SSL, *mut *const u8, *mut usize, *mut c_void) -> i32
        )}),
    },
    OSSL_DISPATCH {
        function_id: OSSL_FUNC_SSL_QUIC_TLS_CRYPTO_RELEASE_RCD,
        function: Some(unsafe { transmute(
            crypto_release_rcd_cb as unsafe extern "C"
            fn(*mut SSL, usize, *mut c_void) -> i32
        )}),
    },
    OSSL_DISPATCH {
        function_id: OSSL_FUNC_SSL_QUIC_TLS_YIELD_SECRET,
        function: Some(unsafe { transmute(
            yield_secret_cb as unsafe extern "C"
            fn(*mut SSL, u32, i32, *const u8, usize, *mut c_void) -> i32
        )}),
    },
    OSSL_DISPATCH {
        function_id: OSSL_FUNC_SSL_QUIC_TLS_GOT_TRANSPORT_PARAMS,
        function: Some(unsafe { transmute(
            got_transport_params_cb as unsafe extern "C"
            fn(*mut SSL, *const u8, usize, *mut c_void) -> i32
        )}),
    },
    OSSL_DISPATCH {
        function_id: OSSL_FUNC_SSL_QUIC_TLS_ALERT,
        function: Some(unsafe { transmute(
            alert_cb as unsafe extern "C"
            fn(*mut SSL, u8, *mut c_void) -> i32
        )}),
    },
    // NULL terminator
    OSSL_DISPATCH {
...
extern "C" fn crypto_send_cb(
    s: *mut SSL, buf: *const c_uchar, buf_len: size_t, consumed: *mut size_t,
    _arg: *mut c_void
) -> c_int {
    let ex_data = match ExData::from_ssl_ptr(s) {
        Some(v) => v,
        None => return 0,
    };

    let ret = super::add_handshake_data(s, ex_data.ossl_data.write_level, buf, buf_len);
    if ret == 1 {
        unsafe { *consumed = buf_len; }
    }

    ret
}

extern "C" fn crypto_recv_cb(
    s: *mut SSL, buf: *mut *const c_uchar, bytes_read: *mut size_t,
    _arg: *mut c_void
) -> c_int {
    let ex_data = match ExData::from_ssl_ptr(s) {
        Some(v) => v,
    ...
    if let Some(data) = ex_data.ossl_data.queued_data.front() {
        unsafe {
            *buf = data.as_ptr();
            *bytes_read = data.len();
        }
...
extern "C" fn crypto_release_rcd_cb(
    s: *mut SSL, bytes_read: size_t, _arg: *mut c_void
) -> c_int {
    let ex_data = match ExData::from_ssl_ptr(s) {
        Some(v) => v,
        None => return 0,
    ...
    let front = match ex_data.ossl_data.queued_data.pop_front() {
        Some(v) => v,
        None => return 0,
    };
    if front.len() != bytes_read {
        return 0;
    }
...
extern "C" fn yield_secret_cb(
    s: *mut SSL, prot_level: c_uint, direction: c_int, secret: *const c_uchar,
    secret_len: size_t, _arg: *mut c_void
) -> c_int {
    let ex_data = match ExData::from_ssl_ptr(s) {
        Some(v) => v,
        None => return 0,
    ...
    let level = match prot_level {
        OSSL_RECORD_PROTECTION_LEVEL_NONE => crypto::Level::Initial,
        OSSL_RECORD_PROTECTION_LEVEL_EARLY => crypto::Level::ZeroRTT,
        OSSL_RECORD_PROTECTION_LEVEL_HANDSHAKE => crypto::Level::Handshake,
        OSSL_RECORD_PROTECTION_LEVEL_APPLICATION => crypto::Level::OneRTT,
        _ => return 0,
    };

    let cipher = map_result_ptr(unsafe { SSL_get_current_cipher(s) });
    match direction {
        0 => {
            ex_data.ossl_data.read_level = level;
            set_read_secret(s, level, cipher.unwrap(), secret, secret_len);
        },
        1 => {
            ex_data.ossl_data.write_level = level;
            set_write_secret(s, level, cipher.unwrap(), secret, secret_len);
        },
        _ => return 0,
    };
...
extern "C" fn got_transport_params_cb(
    s: *mut SSL, params: *const c_uchar, params_len: size_t, _arg: *mut c_void
) -> c_int {
    let ex_data = match ExData::from_ssl_ptr(s) {
        Some(v) => v,
        None => return 0,
    };
    ex_data.ossl_data.peer_transport_params = unsafe {
        slice::from_raw_parts(params, params_len).to_vec().into_boxed_slice()
    };

    1
}

extern "C" fn alert_cb(
    s: *mut SSL, alert_code: c_uchar, _arg: *mut c_void
) -> c_int {
    let ex_data = match ExData::from_ssl_ptr(s) {
        Some(v) => v,
        None => return 0,
    ...
    super::send_alert(s, ex_data.ossl_data.write_level, alert_code)
}
