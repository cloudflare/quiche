...
fn simple_client_driver() {
    ...
    // like receiving a fin. It's a different question if it should treat it as
    // such
...
fn simple_server_driver() {
    ...
    to_client
        .try_send(OutboundFrame::Body(BufFactory::buf_from_slice(&[42]), true))
        .unwrap();
    helper.advance_and_run_loop().unwrap();
...
/// Test the case where the peer sends a STOP_SENDING quiche frame.
#[test]
fn peer_sends_stop_sending_before_our_fin() {
    let mut helper = DriverTestHelper::<ServerHooks>::new().unwrap();
    helper.complete_handshake().unwrap();
    helper.advance_and_run_loop().unwrap();
    ...
    // we read the request and send response headers
    helper.advance_and_run_loop().unwrap();
    let req = assert_matches!(
        helper.driver_recv_core_event().unwrap(),
        H3Event::IncomingHeaders(headers) => { headers }
    ...
    // we need to try and send something to learn about stop_sending
    to_client
        .try_send(OutboundFrame::Body(
...
#[test]
fn peer_sends_stop_sending_after_our_fin() {
    let mut helper = DriverTestHelper::<ServerHooks>::new().unwrap();
    helper.complete_handshake().unwrap();
    helper.advance_and_run_loop().unwrap();
    ...
    to_client
        .try_send(OutboundFrame::Body(
            BufFactory::buf_from_slice(&[7; 5]),
            false,
        ))
        .unwrap();

    // client receives the headers
    helper.advance_and_run_loop().unwrap();
    assert_matches!(
        helper.peer_client_poll(),
        Ok((0, h3::Event::Headers { .. }))
    ...
    // we enqueue a frame with fin
    to_client
        .try_send(OutboundFrame::Body(BufFactory::get_empty_buf(), true))
        .unwrap();
    // write it to quiche but don't send it yet.
    helper.work_loop_iter().unwrap();

    // Client sends STOP_SENDING which is a no-op
    assert_eq!(
        helper
            .pipe
            .client
    ...
    // The client still needs to close its side with a fin
    helper.peer_client_send_body(0, &[1, 2, 3], true).unwrap();
    helper.advance_and_run_loop().unwrap();
    let (body, fin, _) = helper.driver_try_recv_body(&mut from_client);
...
/// Test the case where the peer sends a RESET_STREAM quiche frame.
/// The peer sends its reset before we send a fin
#[test]
fn peer_sends_reset_stream_before_our_fin() {
    let mut helper = DriverTestHelper::<ServerHooks>::new().unwrap();
    helper.complete_handshake().unwrap();
    helper.advance_and_run_loop().unwrap();
    ...
    // The driver responded with a RESET and closed the outbound channel
...
/// Test the case where the peer sends a RESET_STREAM quiche frame.
/// We send a fin before the peer sends reset
#[test]
fn peer_sends_reset_stream_after_our_fin() {
    let mut helper = DriverTestHelper::<ServerHooks>::new().unwrap();
    helper.complete_handshake().unwrap();
    helper.advance_and_run_loop().unwrap();
...
/// Test the case where the peer sends a RESET_STREAM quiche frame while we're
/// in the middle of reading data. We want to excercise the code-path where
/// `upstream_ready` is called before `process_reads`. If `process_reads()` is
/// called first, it will get the Reset event. If `upstream_ready()` is called
/// first, it will attempt to read from the h3::Connection and will get a
/// `TransportError(StreamReset(code))`
#[test]
fn peer_sends_reset_stream_while_reading_wait_for_data() {
    let mut helper = DriverTestHelper::<ServerHooks>::new().unwrap();
    helper.complete_handshake().unwrap();
    helper.advance_and_run_loop().unwrap();
    ...
    // The channels are closed because the peer send us the reset and
    // sent a reset back
    assert!(from_client.is_closed());
    assert!(to_client.is_closed());
    assert_eq!(helper.driver.stream_map.len(), 0);
    ...
    assert_eq!(audit_stats.downstream_bytes_sent(), 4);
}

/// Test the case where the peer sends a RESET_STREAM quiche frame while we're
/// in the middle of reading data. We want to excercise the code-path where
/// where we call `process_reads` before `upstream_ready()`.
#[test]
fn peer_sends_reset_stream_while_reading_process_reads() {
    let mut helper = DriverTestHelper::<ServerHooks>::new().unwrap();
    helper.complete_handshake().unwrap();
    helper.advance_and_run_loop().unwrap();
...
fn client_send_reset_stream_when_outbound_frame_channel_drops() {
    ...
    // server receives the reset, and send a body back
    assert_eq!(
...
fn client_send_reset_stream_with_full_stream() {
    ...
    assert!(resp.read_fin);
    let audit_stats = resp.h3_audit_stats.clone();
    // send a body the to server, but not enough flow control for the full body
    resp.send
        .get_ref()
        .unwrap()
        .try_send(OutboundFrame::Body(
...
fn server_driver_send_stop_sending_after_channel_drop() {
    ...
    // servers reads request and sends response headers
    helper.advance_and_run_loop().unwrap();
    let req = assert_matches!(helper.driver_recv_core_event().unwrap(), H3Event::IncomingHeaders(headers) => { headers
    });
