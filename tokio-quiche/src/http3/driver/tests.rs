...
fn peer_sends_stop_sending() {
    ...
    // The way quiche is implemented, we need to attempt a write to the stream to
    // learn that it's closed. So we add an OutboundFrame to the channel and let
    // the driver write it. The driver gets a StreamStopped back and closes the
    // channel.
    to_client
        .try_send(OutboundFrame::Body(
    ...
    assert_eq!(audit_stats.sent_stream_fin(), StreamClosureKind::Implicit);
...
fn peer_sends_reset_stream_before_our_fin() {
    ...
    // We can still write to the peer and in fact, we must eventually send a fin.
    to_client
        .try_send(OutboundFrame::Body(
    ...
    assert_eq!(audit_stats.sent_reset_stream_error_code(), -1);
    assert_eq!(audit_stats.sent_stop_sending_error_code(), -1);
    assert_eq!(audit_stats.recvd_stream_fin(), StreamClosureKind::Implicit);
