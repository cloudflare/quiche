...
Alternatively you can use [QuicTLS] or [OpenSSL]. To enable quiche to use these
vendors, the ``quictls`` or ``openssl`` feature can be added to the
``--features`` list. Be aware that ``0-RTT`` is not supported if these vendors
are used.
